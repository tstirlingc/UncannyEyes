...
#define IRIS_MIN      120 // Clip lower analogRead() range from IRIS_PIN (120)
#define IRIS_MAX      300 // Clip upper " (300)
//#define IRIS_MIN      70 // Clip lower analogRead() range from IRIS_PIN
...
#define EYELID_COLOR 0 // black [default]
//#define EYELID_COLOR 0xF800 // Red first 5 bits
//#define EYELID_COLOR 0x07e0 // Green second 6 bits
//#define EYELID_COLOR 0x001F // Blue last 5 bits
//#define EYELID_COLOR 0xAB4B // skin (from Dumbledore picture R,G,B = 171,104,90 -> AB,68,5A -> AB4B)
...
void frame( // Process motion for a single frame of left or right eye
  ...
  if(eye[eyeIndex].blink.state) { // Eye currently blinking?
    ...
    if((t - eye[eyeIndex].blink.startTime) >= static_cast<uint32_t>(eye[eyeIndex].blink.duration)) {
  ...
  if(eye[eyeIndex].blink.state) { // Eye currently blinking?
    ...
    if(s >= static_cast<uint32_t>(eye[eyeIndex].blink.duration)) s = 255;   // At or past blink end
